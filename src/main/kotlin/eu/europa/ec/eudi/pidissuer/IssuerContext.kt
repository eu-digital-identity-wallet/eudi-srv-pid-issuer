    ...
    BeanRegistrarDsl {
        ...
        val issuerKeystore: KeyStore by lazy {
            ...
            checkNotNull(keystoreResource) {
                "Could not load Keystore either from '$keystoreLocation' or '$KEYSTORE_DEFAULT_LOCATION'"
            }
                ...
                val key =
                    when (env.getProperty<KeyOption>("issuer.credentialRequestEncryption.jwks")) {
                        ...
                        KeyOption.LoadFromKeystore -> {
                            ...
                            val keyAlgorithm =
                                env
                                    .getProperty<String>("issuer.credentialRequestEncryption.jwks.algorithm")
        ...
        registerBean(lazyInit = true) {
            val signedMetadataIssuer =
                env
                    ...
                    ?.trim()
                    ?.let {
                        requireNotNull(HttpsUrl.of(it)) {
                            "'issuer.signed-metadata.issuer' is not a valid HttpsUrl"
                        }
                    }
                    ?: issuerPublicUrl
        ...
        registerBean {
            if (trustValidatorServiceUrl.isNullOrBlank()) {
                ...
                IsTrustedKeyAttestationIssuer.usingTrustValidatorService(
                    bean(),
                    URI.create(trustValidatorServiceUrl),
                )
            }
        }
                    ...
                    buildList {
                        if (enableMsoMdocPid) {
                            ...
                            val keyAttestationRequirement =
                                this@BeanRegistrarDsl.keyAttestationRequirement("issuer.pid.mso_mdoc")
                        ...
                        if (enableSdJwtVcPid) {
                            ...
                            val keyAttestationRequirement =
                                this@BeanRegistrarDsl.keyAttestationRequirement("issuer.pid.sd_jwt_vc")
                        ...
                        if (enableLearningCredential) {
                            ...
                            val keyAttestationRequirement =
                                this@BeanRegistrarDsl.keyAttestationRequirement("issuer.learningCredential")
...
private fun BeanRegistrarDsl.keyAttestationRequirement(attestationPropertyPrefix: String): KeyAttestationRequirement {
    ...
    val keyStorageConstraints =
        env.getProperty<List<String>>(
            "$attestationPropertyPrefix.key_attestations.constraints.key_storage",
        )
    val userAuthenticationConstraints =
        env.getProperty<List<String>>(
...
private fun Environment.readRequiredUrl(
    key: String,
...
private fun <T> Environment.readNonEmptySet(
    key: String,
    f: (String) -> T?,
): NonEmptySet<T> {
    val nonEmptySet =
...
private fun <T> Environment.readNullableNonEmptySet(
    key: String,
    f: (String) -> T?,
): NonEmptySet<T>? =
