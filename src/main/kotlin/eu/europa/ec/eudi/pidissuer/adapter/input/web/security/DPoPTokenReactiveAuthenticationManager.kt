 ...
 */
private suspend fun DPoPNoncePolicy.verifyDPoPNonce(unvalidated: String?, now: Instant): Nonce? =
    when (this) {
        DPoPNoncePolicy.Disabled ->
            if (unvalidated != null) throw OAuth2AuthenticationException(DPoPTokenError.invalidToken("DPoP Nonce is not allowed"))
            else null

        is DPoPNoncePolicy.Enforcing ->
            if (unvalidated == null) throw OAuth2AuthenticationException(DPoPTokenError.useDPoPNonce("Invalid DPoP proof."))
            else {
                if (verifyDPoPNonce(unvalidated, now)) Nonce(unvalidated)
