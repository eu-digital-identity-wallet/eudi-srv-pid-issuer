...
internal class CreateDidUrlTest {

    @Test
    fun `verify did key method resolution success`() {
        testData.forEach { (didUrl, jwk) ->
            val method = if (didUrl.contains("key")) DidMethod.KEY else DidMethod.JWK
            val result = createDidUrl(jwk, method).getOrThrow()
            assertEquals(didUrl, result.toString())
        }
    }

    @Test
    fun `verify did url creation and resolution is symmetric`() {
        val jwkStr =
            """
                {
                    "kty": "RSA",
        ...
        val jwk = assertIs<RSAKey>(JWK.parse(jwkStr))
        val didUrlFromJwk = createDidUrl(jwk, DidMethod.KEY).getOrThrow()
        val jwkFromDidUrl = assertIs<RSAKey>(resolveDidUrl(didUrlFromJwk).getOrThrow())

        assertEquals(jwkFromDidUrl.toPublicJWK().modulus, jwk.toPublicJWK().modulus, "Non symmetric modulus")
        assertEquals(jwkFromDidUrl.toPublicJWK().publicExponent, jwk.toPublicJWK().publicExponent, "Non symmetric modulus")
    }
    ...
    private val testData = mapOf(
        ...
        "did:key:${"zgghBUVkqmWS8e1ioRVp2WN9Vw6x4NvnE9PGAyQsPqM3fnfPf8EdauiRVfBTcVDyzhqM5FFC7ekAvuV1cJHawtfgB9wDcru1hPD" +
            ...
            """.trimIndent(),
        "did:jwk:${"eyJrdHkiOiJFQyIsImNydiI6IlAtMjU2IiwieCI6ImFjYklRaXVNczNpOF91c3pFakoydHBUdFJNNEVVM3l6OTFQSDZDZEgyVjAiLCJ5I" +
            "joiX0tjeUxqOXZXTXB0bm1LdG00NkdxRHo4d2Y3NEk1TEtncmwyR3pIM25TRSJ9"}" to
            """
                {
                    "crv": "P-256",
                    "kty": "EC",
            ...
            """.trimIndent(),
        "did:jwk:${"eyJrdHkiOiJPS1AiLCJ1c2UiOiJlbmMiLCJjcnYiOiJYMjU1MTkiLCJ4IjoiM3A3YmZYdDl3YlRUVzJIQzdPUTFOei1EUThoYm" +
