...
class ValidateProofTest {
    ...
    @Test
    internal fun `fails with unsupported proof type`() =
        runTest {
    ...
    internal fun `keys are not truncated when reuse policy is None`() =
        runTest {
            ...
            val result =
                runValidateProofsKeepingAllKeys(
                    proofs = proofs,
    ...
    @Test
    internal fun `truncation spans multiple proofs`() =
        runTest {
    ...
    private suspend fun runValidateProofsKeepingAllKeys(
        ...
        val extract =
            ExtractJwkFromCredentialKey { credentialKey ->
    ...
    private fun generateEcKey(): ECKey = ECKeyGenerator(Curve.P_256).generate()

    private fun generateJwk(): JWK = ECKeyGenerator(Curve.P_256).generate().toPublicJWK()
}
